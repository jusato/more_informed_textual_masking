...
class Trainer(object):
    ...
    def __init__(self, data, params):
        ...
        # same for regions
        # keep <do nothing> as a choice as well to simplify image masking ops
        params.region_pred_probs = torch.FloatTensor(
            [0.0, params.region_mask, params.region_keep, params.region_rand])
    ...
    def convert_to_text(self, batch, lengths, dico, params):
        """
        ...
        for j in range(bs):
            for k in range(1, lengths[j]):
                if batch[k, j] == params.eos_index:
                    break
                word = dico[batch[k, j]]
                if self.is_fem_masc_word(word):
                    femMasc_x[k, j] = 1
                    count_femMasc += 1
                elif self.is_fem_masc_parcial_word(word) and k + 1 < lengths[j] and self.is_fem_masc(dico[batch[k+1, j]]):
                    femMasc_x[k, j] = 1
                    femMasc_x[k+1, j] = 1
                    count_femMasc += 2
        return femMasc_x, count_femMasc

    def is_fem_masc_word(self, word):
        return word in ['ela', 'ele']

    def is_fem_masc_parcial_word(self, word):
        return word in ['del@@', 'su@@', 'se@@', 'el@@']

    def is_fem_masc(self, word):
        return word in ['a', 'e', 'u']

    def mask_out(self, x, lengths, dico, params):
        """
        Decide of random words to mask out, and what target they get assigned.
        """
        slen, bs = x.size()
        if self.params.sample_alpha == 0:
            femMasc_x, count_femMasc = self.convert_to_text(x, lengths, dico, params)
            ...
            for j in range(bs):
                for k in range(1, lengths[j]):
                    if femMasc_x[k, j] == 1 and pred_mask[k, j] == 1:
                        count += 1
                        if k+1 < lengths[j] and femMasc_x[k+1, j] == 1 and pred_mask[k+1, j] == 0:
                            pred_mask[k+1, j] = 1
                            count += 1
                            remove += 1
                        elif k-1 > 0 and femMasc_x[k-1, j] == 1 and pred_mask[k-1, j] == 0:
                            pred_mask[k-1, j] = 1
                            count += 1
                            remove += 1
                    elif pred_mask[k, j] == 1 and remove > 0:
                        pred_mask[k, j] = 0
                        remove -= 1
            ...
            if remove > 0:
                for j in range(bs):
                    for k in range(1, lengths[j]):
                        if pred_mask[k, j] == 1 and femMasc_x[k, j] == 0:
                            pred_mask[k, j] = 0
                            remove -= 1
                        if remove <= 0:
                            break
            ...
            if perc < 0.05:
                new_perc = 0.05 - perc
                to_be_added = min(new_perc * (bs*slen), count_femMasc - count)
                to_be_removed = to_be_added
                for j in range(bs):
                    for k in range(1, lengths[j]):
                        if to_be_removed > 0 and pred_mask[k, j] == 1 and femMasc_x[k, j] == 0:
                            pred_mask[k, j] = 0
                            to_be_removed -= 1
                        elif to_be_added > 0 and pred_mask[k, j] == 0 and femMasc_x[k, j] == 1:
                            pred_mask[k, j] = 1
                            to_be_added -= 1
                            if k+1 < lengths[j] and femMasc_x[k+1, j] == 1 and pred_mask[k+1, j] == 0:
                                pred_mask[k+1, j] = 1
                                ...
                                if to_be_added > 0:
                                    ...
                                    to_be_removed += 1
                        if to_be_removed <= 0 and to_be_added <= 0:
                            break
                    if to_be_removed <= 0 and to_be_added <= 0:
                        break
    ...
    def mask_out_image(self, img_boxes, img_feats, img_labels):
        ...
        # 1: multiply the positions' features with 0 + predict labels (mask)
        # 2: no change at input + predict labels (keep)
        # 3: random features at input + predict labels (rand)
        probs = torch.multinomial(
            self.params.region_pred_probs, bs * n_regions, replacement=True)
        ...
        # dimension incompatibility (512 != 1536). So let's make them 0 now
        # and then handle in transformer.py:fwd() if <MASK> is indeed requested
    ...
    def vlm_step(self, lang1, lang2, lambda_coeff, iter):
        ...
        x, y, txt_pred_mask = self.mask_out(x, lengths, self.data['dico'], params)
        img_boxes, img_feats, img_y, img_pred_mask, img_mask_pos = self.mask_out_image(
        ...
        img_boxes, img_feats, img_labels, img_y, img_pred_mask, img_mask_pos = to_cuda(
            img_boxes, img_feats, img_labels, img_y, img_pred_mask, img_mask_pos)

        # forward / loss
        tensor = model(
            'fwd', x=x, lengths=lengths, positions=positions, causal=False,
            langs=langs, image_langs=image_langs,
            img_boxes=img_boxes, img_feats=img_feats, img_mask_pos=img_mask_pos)
